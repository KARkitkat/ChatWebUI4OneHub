#!/usr/bin/env bash
# TopAI Docker 启动脚本 - Linux 服务器部署
# 使用 docker-compose.linux.yml，兼容旧版 docker-compose（3.3、无 platform）
...
case "${1:-start}" in
  start)
    echo "[Linux] 使用 $COMPOSE_FILE 启动..."
    docker-compose -f "$COMPOSE_FILE" up -d
    echo "启动完成。应用: http://localhost（或服务器 IP）"
